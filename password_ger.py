...
# ── SVG icons ──
ICON_KEY   = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><circle cx="7" cy="17" r="3"/><path d="M10 17h10M17 14v6"/><path d="m10 14 7-7"/></svg>'
...
def strength_bars_html(filled, color):
    return "".join(
        f'<div class="s-bar" style="background:{color if i < filled else "var(--border)"}\x3c/div>'
        for i in range(5)
    )
